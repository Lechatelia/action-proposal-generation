...
def video_post_process(opt, video_list, video_dict):
    for video_name in video_list:
        ...
        if len(df) > 1:
            snms_alpha = opt["bmn_soft_nms_alpha"]
            snms_t1 = opt["bmn_soft_nms_low_thres"]
            snms_t2 = opt["bmn_soft_nms_high_thres"]
...
def BMN_post_processing(opt):
    ...
    num_videos_per_thread = num_videos // opt["bmn_post_process_thread"]
    processes = []
    for tid in range(opt["bmn_post_process_thread"] - 1):
        ...
        p.start()
        processes.append(p)
    tmp_video_list = video_list[(opt["bmn_post_process_thread"] - 1) * num_videos_per_thread:]
    p = mp.Process(target=video_post_process, args=(opt, tmp_video_list, video_dict))
    p.start()
    processes.append(p)
    for p in processes:
    ...
    outfile = open(opt["bmn_result_file"], "w")

...
def PEM_loss_function(anchors_iou, match_iou, model, opt):
    ...
    u_hmask = (match_iou > opt["pem_high_iou_thres"]).float()  # > positive
    u_mmask = ((match_iou <= opt["pem_high_iou_thres"]) & (match_iou > opt["pem_low_iou_thres"])).float()
    u_lmask = (match_iou < opt["pem_low_iou_thres"]).float()  # negative

    num_h = torch.sum(u_hmask)  # positive
    num_m = torch.sum(u_mmask)
    num_l = torch.sum(u_lmask)  # negative

    # 通过下面的采样过程，达到三部分最终的比例是 1：r_m：r_l
    r_m = model.module.u_ratio_m * num_h / (num_m) # 保留下来的m部分 r_m * num_m = u_ratio_m * num_h
    r_m = torch.min(r_m, torch.Tensor([1.0]).cuda())[0] # 如果不够也就没办法了
    u_smmask = torch.Tensor(np.random.rand(u_hmask.size()[0])).cuda() # np.random.rand 从 0 1之间均值采样
    u_smmask = u_smmask * u_mmask
    u_smmask = (u_smmask > (1. - r_m)).float() # 通过比较达到随机采样的目的，从而保留下来的比例是r_m

    r_l = model.module.u_ratio_l * num_h / (num_l) # 保留下来的l部分 r_l * num_l = u_ratio_l * num_h
